    ...
    >
      ...
      <div className="absolute inset-x-3 bottom-3 border border-white/60 bg-white/88 p-3 backdrop-blur">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {industry.visualTheme.previewItems.slice(0, compact ? 3 : 4).map((item) => (
            <span
              key={item}
...
export function EditorialCard({ post }: { post: BlogPost }) {
  return (
    ...
    >
      ...
      <h2 className="mt-4 text-2xl font-semibold group-hover:text-[var(--coral-strong)]">
        {post.title}
      </h2>
      <p className="mt-4 leading-7 text-[var(--muted)]">{post.description}</p>
      <div className="mt-6 grid grid-cols-3 gap-2">
        <span className="h-2 bg-[var(--coral)]" />
        <span className="h-2 bg-[var(--foreground)]" />
        <span className="h-2 bg-[var(--border)]" />
      </div>
    </Link>
  );
}
